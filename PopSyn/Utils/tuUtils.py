...
def numerical_binning(df, cols, q=5):
    ...
    for var in cols:
        try:
            pd.qcut(df[var], q=2)
            # If you want to force it to be of q categories, delete duplicates='drop'
            df[var] = pd.cut(df[var], bins=np.quantile(df[var], q=np.linspace(
                0, 1, num=q)), duplicates='drop', include_lowest=True)
        except:
...
def encode_onehot(df, cols=None):
    ...
    if cols is None:
        cols = list(df)
    # The sparse=True option might be taken off when using the server
    df = pd.get_dummies(data=df, columns=cols, sparse=True)
    return df
...
def back_from_dummies(df):
    ...
    dummmy_tuples = [(col.split("_")[0], col)
...
def data_creator(df, numerical, train_prop=0.5, val_prop=0.5, binning=True, condition_on=None, quantiles=15):
    ...
    if condition_on is not None:
        ...
        # Remove conditioning variable from the list
        cols.pop(cols.index(condition_on))
        # Create a new dataframe with the conditioning column at the end
        total_df = total_df[cols+[condition_on]]
        # Categories of variable of interest
        cond_labels = total_df[condition_on].cat.categories

    # Binning or scaling numerical variables
    if numerical is not None:
        if binning:
            pre_one_hot_df = numerical_binning(
                total_df, numerical, q=quantiles)
        else:
    ...
    categorical = [col for col in list(df) if col not in numerical]
    one_hot_df = encode_onehot(pre_one_hot_df, categorical)  # One hot encoding variables
    col_names = one_hot_df.columns

    print(col_names)

    validation, train, test = np.split(one_hot_df.sample(frac=1).values, [int(
        val_prop*train_prop*len(df)), int(train_prop*len(df))])  # Splitting into train, validation and test

    print('Train shape is:', train.shape)
    print('Validation shape is:', validation.shape)
    print('Test shape is:', test.shape)
    return train, test, validation, pre_one_hot_df, one_hot_df, col_names, scaler


def samples_to_df(samples, col_names, original_df, pre_one_hot_df, print_duplicates=False, binning=True, numerical=None, scaler=None):
    r"""
    ...
    #scaler.fit(original_df[numerical])
    # Converting array into df, uses the column names AFTER one hot encoding
    total_samp_df = pd.DataFrame(data=samples.transpose(), columns=col_names)
    # Going back from dummies to single categorical variables
    total_samp_df = back_from_dummies(total_samp_df)
    if not binning:
        #total_samp_df[numerical] = scaler.inverse_transform(
            #total_samp_df[numerical])  # Destandarizing numerical variables
        total_samp_df=transform_numerical(total_samp_df, numerical, scaler)
    # Defining categorical variables
    object_cols = total_samp_df.columns[total_samp_df.dtypes == 'object']
    total_samp_df[object_cols] = total_samp_df[object_cols].astype(
        'category')  # Converting their dtype

    for var in object_cols:
        if binning:
            original_cats = [str(cat) for cat in list(
                pre_one_hot_df[var].cat.categories)]
        else:
            original_cats = [str(cat) for cat in list(
                original_df[var].cat.categories)]
        total_samp_df[var] = total_samp_df[var].cat.add_categories(
            [cat for cat in original_cats if cat not in total_samp_df[var].cat.categories])
...
def transform_numerical(df, numerical, scaler):
    """
    Transfomrs numerical values back 

    Arguments:
            df: Dataframe that should be transformed 
            numerical: Column names of numerical values
            scaler: Instnace of the scaler used to transform the numercal values
    """
    numerical_columns = df[numerical].columns

    # Perform the inverse transform
    transformed_data = scaler.inverse_transform(df[numerical])
    ...
    df[numerical] = result_df

    return df

def data_creator_noSplitting(df, numerical, binning=False):
    r"""
    Function that takes a dataset, bins or standarizes its numerical variables, and splits into train validation and test sets
    ...
    print('Train shape is:', train.shape)
    return train, pre_one_hot_df, one_hot_df, col_names, scaler
