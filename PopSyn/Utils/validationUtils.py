...
from Utils.tuUtils import samples_to_df
...
def evaluate_DATGAN(real_df, model_df, agg_vars, plot=False):
    # Calculate counts for model and real dataframes
    model_df['count'] = 1
    model_df = model_df.groupby(agg_vars, observed=True).count()
    model_df /= model_df['count'].sum()
    ...
    N = 1
    for var in agg_vars:
        N *= len(real_df[var].cat.categories) 
    ...
    return real_and_sampled, metrics
